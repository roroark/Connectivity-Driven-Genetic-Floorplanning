...
Rect MaxRectsBinPack::Insert(double width, double height, FreeRectChoiceHeuristic method)
{
	Rect newNode;
	...
	switch(method)
	{
		//RYAN FUTURE MAYBE TODO: Look into these individual functions for the treatment of score1 and score2.
...
void MaxRectsBinPack::Insert(std::vector<RectSize> &rects, std::vector<Rect> &dst, FreeRectChoiceHeuristic method)
{
	...
	while(rects.size() > 0)
	{
		double bestScore1 = std::numeric_limits<int>::max();
		double bestScore2 = std::numeric_limits<int>::max();
	...
	double &bestShortSideFit, double &bestLongSideFit) const
{
	...
	for(size_t i = 0; i < freeRectangles.size(); ++i)
	{
		...
		if (freeRectangles[i].width >= width && freeRectangles[i].height >= height)
		{
			...
			if (bestShortSideFit == -1 || shortSideFit < bestShortSideFit || (shortSideFit == bestShortSideFit && (bestLongSideFit == -1 || longSideFit < bestLongSideFit)))
			{
				bestNode.x = freeRectangles[i].x;
				bestNode.y = freeRectangles[i].y;
				bestNode.width = width;
		...
		if (binAllowFlip && freeRectangles[i].width >= height && freeRectangles[i].height >= width)
		{
			...
			if (bestShortSideFit == -1 || flippedShortSideFit < bestShortSideFit || (flippedShortSideFit == bestShortSideFit && (bestLongSideFit == -1 || flippedLongSideFit < bestLongSideFit)))
			{
				bestNode.x = freeRectangles[i].x;
				bestNode.y = freeRectangles[i].y;
				bestNode.width = height;
